...
$user["login_user"] = $_SESSION["login_user"];
$posts = new DreamsharingController();
$params = $posts->index_control();
...
<body>
  ...
  <div class="container">
    ...
    <div class="row">
      <?php foreach($params["postAll"] as $post):?>
        <div class="col-md-5 index-item">
          ...
          <?php $like = $posts->likes($post["id"]); ?>
          <p>いいね！：<?= $like["count(post_id)"]; ?> 件　　　　　<a href="detail.php?id=<?=$post["id"] ?>">詳細はこちら、、、</a></p>
        </div>
      <?php endforeach;?>
    </div>
